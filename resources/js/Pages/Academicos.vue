...
<app-layout>
    <template #header>
      ...
      <ul class="list-reset flex">
        <li class="mr-3">
            <a @click="listadoAlumnos()" v-bind:class="alumnoListadoClass" href="#">Listado</a>
        </li>
        <li class="mr-3">
            <a @click="crearAlumnos()" v-bind:class="alumnoCrearClass" href="#">Crear</a>
        </li>
        </ul>
    </h2>
    ...
    <div class="py-12" v-if="enListadoAlumno">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
              ...
              <tbody>
                <tr v-for="alumno in academicos" :key="alumno.id">
                  <td>
                          {{ alumno.id }}
                  </td>
                  <td>
            ...
            <div class="m-7">
                <form v-on:submit.prevent>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea rows="5" v-model="form.observacion" name="observaciones" placeholder="Telefono, Otro mail" class="w-full px-3 py-2 placeholder-gray-300 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500 dark:border-gray-600 dark:focus:ring-gray-900 dark:focus:border-gray-500" required></textarea>
                        <div v-if="errors.observacion" class="text-red-500">{{ errors.observacion }}</div>
                    </div>
                    <div class="mb-6">
                        <button type="buton" v-show="!editFormAlumno" @click="saveAlumno(form)" class="w-full px-3 py-4 text-white bg-indigo-500 rounded-md focus:bg-indigo-600 focus:outline-none">Crear nuevo</button>
                        <button type="button" v-show="editFormAlumno" @click="updateAlumno(form)" class="w-full px-3 py-4 text-white bg-red-500 rounded-md focus:bg-indigo-600 focus:outline-none">Actualizar</button>
      ...
      return {
        ...
        desactivo:'inline-block border border-white rounded hover:border-grey-lighter text-blue hover:bg-grey-lighter py-1 px-3',
          isOpenFormAlumno:false,
            ...
            alumnoListadoClass:'inline-block border border-blue rounded hover:border-grey-lighter text-blue hover:bg-grey-lighter py-1 px-3',
  ...
  methods:{
      openFormAlumno:function(d){
        ...
        if(d){
          ...
          this.form.titulo='Editar';
          this.form.descripcion='Editar datos';
          this.editFormAlumno=true;
        }else{
          this.form.titulo='Crear nuevo  alumno';
          this.form.descripcion='Llenar campos requeridos';
          this.editFormAlumno=false;
        }
      },
      saveAlumno(d){
        this.$inertia.post('/academicos/',d);
      },
      updateAlumno(d){
        //console.log(d);
        var x=d;
        this.$inertia.put('/academicos/'+this.form.id,x);
      },
      eliminaAlumno(d){
          if (!confirm('¿Eliminar ? '+d.nombre)) return;
        d._method = 'DELETE';
        this.$inertia.post(
          route("academicos.destroy", d.id),
          d
        );
      },
      listadoAlumnos(){
